        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <table id="tabel-detail" class="table table-sm table-bordered" style="width: 100%;display:none;">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th>Jurusan Diterima</th>
                                <td id="o-jurusanditerima" class="o-detaildaftar"></td>
                            </tr>
                        </thead>
                    </table>
                    ...
                    <div id="test-navigation" style="display: none;" class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <form id="form-diterima" style="display: none; margin-top:20px;" method="POST" action="#">
                        @csrf
                        <input type="hidden" id="kodedaftar" name="kodedaftar" value="">
                        <input type="hidden" id="status_diterima" name="status_diterima" value="">
                        <label for="jurusan_diterima">Tentukan Jurusan Diterima</label>
        ...
        $(function() {
            ...
            function detailhasil_test() {
                $('.btn_detail').off('click').on('click', function (e) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '{!! url('admin/TestAssesment/DetailTestOnlinePMB') !!}' + '/' + params,
                        dataType: "JSON",
                        beforeSend: function() {
                            $('#loading').show();
                        ...
                        success: function(data) {
                            ...
                            if(data.hasil == 0){
                                ...
                                $('#judul-modal').html('Detail Hasil Assessment');
                                ...
                                if(data.attempts && data.attempts.length > 0) {
                                    ...
                                    $('#assessment-container').html('<div class="alert alert-warning text-center">Belum ada data pengerjaan test (attempts).</div>').show();
                                }
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if(data.hasil == 0){
                                ...
                                $('#judul-modal').html('Form Kelulusan Test');
                                $('#btn-lolos').show();
                                $('#kodedaftar').val(params);
                                $('#status_diterima').val('1');

                                let html = '<option value="" selected disabled>-- Pilih Jurusan --</option>';
                                for (let i = 0; i < data.jurusan.length; i++) {
                                    html += '<option value="'+data.jurusan[i].KodeJurusan+'">'+data.jurusan[i].jenjang.jenjang+' - '+data.jurusan[i].jurusan+'</option>';
                                }
                                $('#jurusan_diterima').html(html);
                    ...
                    if($('#jurusan_diterima').val()==''||$('#jurusan_diterima').val()==null){
                        ...
                        Swal.fire({
                            ...
                            if(result.value){
                                $.ajax({
                                    type: "POST",
                                    url: "{{route('admin.testassesment.save')}}", // Ubah pastikan panggil route yg benar
                                    data: dataku,
                                    dataType: "JSON",
                ...
                $('.tidaklolos').click(function (e) {
                    ...
                    Swal.fire({
                        ...
                        if(result.value){
                            $.ajax({
                                type: "POST",
                                url: "{{route('admin.testassesment.save')}}", // Sesuaikan nama route-nya
                                processData: false,
                                contentType: false,
                                data: formData,
                                dataType: "JSON",
                                ...
                                success: function(data) {
                                    $('#loading').hide()
                                    Swal.fire({ title: data.title, text: data.message, icon: data.status }).then(() => {
                                        if(data.status=='success') $('#example2').DataTable().ajax.reload();
                                    });
                                },
                                error: function() {
                                    $('#loading').hide()
