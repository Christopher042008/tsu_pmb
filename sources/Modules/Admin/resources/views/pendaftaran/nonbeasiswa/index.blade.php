...
@section('script')
    <script>
        $(function() {
            ...
            function ShowDetail() {
                // Menggunakan delegasi event agar tombol dalam DataTables terbaca sempurna
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if(data.hasil == 0) {
                                ...
                                // --- PENGAMAN SEMUA DATA MENGGUNAKAN TERNARY CHECK (? ... : '-') ---
                                ...
                                // Karena ini Non-Beasiswa, kita kosongkan saja field beasiswanya
                                $('#o-beasiswa').html('-');
                                $('#o-juarabea').html('-');
                                $('#o-keteranganbea').html('-');
                                $('#o-durasid3').html('-');
                                $('#o-durasis1').html('-');

                                // PENGAMAN LIST BERKAS
                                let berkas = '';
                                if (data.daftar.jalur && data.daftar.jalur.berkasumum) {
                                    berkas += '<tr style="background-color: rgb(0, 238, 255);"><th colspan="4" class="text-center">' + data.daftar.jalur.berkasumum.jenis_berkas + '</th></tr>';
                                    berkas += '<tr style="background-color: rgb(0, 238, 255);"><th>No</th><th>Nama berkas</th><th>Keterangan</th><th>Format File</th></tr>';

                                    for (let i = 0; i < data.daftar.jalur.berkasumum.berkas.length; i++) {
                                        let no = i + 1;
                                        berkas += '<tr>' +
                                            '<td>' + no + '</td>' +
                                ...
                                if (data.daftar.jalur && data.daftar.jalur.berkaskhusus) {
                                    ...
                                    berkas += '<tr><th colspan="4" class="text-center">Tidak Ada Berkas Khusus</th></tr>';
                                }

                                $('#detail-berkas').append(berkas);

                                // AKHIRNYA BISA MEMUNCULKAN MODAL DENGAN AMAN
